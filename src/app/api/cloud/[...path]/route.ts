...
async function handle(request: Request) {
  if (!cloudConfigured()) {
    if (request.method === "GET" && new URL(request.url).pathname === "/api/cloud/account") return Response.json({ configured: false, account: null }, { headers: { "Cache-Control": "no-store" } });
