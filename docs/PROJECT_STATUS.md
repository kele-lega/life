...
The direct `/lab/life-extraction` route now restores the latest scratch input, Proposals, review states, and materialized Events after refresh. It warns that Accept/Correct creates real LifeEvents that can affect Life Map. Statistics still reads only source-valid final LifeEvents and exposes the same Summary/Time Series/Exploration structures; it never reads Proposal data, and Life Map has no AI-specific branch. Real providers, automatic extraction, workers, save hooks, homepage AI, and original-record changes remain absent.

The repeatable fake-indexeddb benchmark migrated and indexed 50,000 v5 LifeEvents in 1017.79ms and reopened v6 in 0.31ms on this development environment. This is not a browser/device SLA and does not justify additional indexes. Final verification passed: `npm run typecheck`; zero-warning `npm run lint`; 34 files / 258 unit-integration tests; 44/44 Chromium E2E tests; and the Next.js production build with `/lab/life-extraction` and `/life` prerendered. The performance harness separately passed all four migration/statistics cases.

Phase 14.2 Life Intelligence Lab Review adds the direct `/lab/life-extraction` experiment surface. An explicit button runs the deterministic fake extractor over entered text and date/timezone context. The page preserves the extracted source snapshot, shows each structured proposal, and sends Accept, editable Correct and Reject actions through the Phase 14.1 state machine/materialization service. Final contract objects display origin, event fields and proposal provenance.

The lab uses a session-only `LabMemoryLifeIntelligenceRepository`. It does not import Dexie or the current LifeEvent repository, call a network service, modify Moment/Diary, start automatic extraction or feed Statistics/Life Map. Refresh intentionally clears jobs, proposals and materializations; this is stated on the page. Manual-conflict checks cover corrected manual materializations created during the current page session only.

Phase 14.2 verification: `npm run typecheck` passed; `npm run lint` passed with zero warnings; `npm test` passed 34 files / 245 tests; a clean full rerun of `npm run test:e2e` passed 44/44 Chromium tests; `npm run build` passed and prerendered `/lab/life-extraction`. The new browser test verifies Accept, narrow-screen reflow, no `life` IndexedDB creation and refresh reset. An earlier full E2E attempt observed one transient existing Life Map label-overlap assertion; that exact test and the complete unchanged suite both passed on rerun without modifying or weakening the Life Map check.

Phase 14.1 Life Intelligence Contract is implemented as a non-persistent domain boundary. It defines extraction jobs, proposals, terminal review transitions, insert-only LifeEvent materialization plans, a storage port and a deterministic fake extractor. Accept plans an AI-origin event, correction plans a manual-origin event, and rejection plans no event; manual conflicts block materialization. No contract object is stored in IndexedDB in this phase.

Dexie remains v5. There is no migration, AI provider, network request, route, homepage entry, automatic save hook or background task. Moment, MomentAppend, Diary, the physical manual-only LifeEvent schema/repository, Life Statistics and Life Map are unchanged. A later approved schema phase is required before accepted or corrected plans can become real persisted LifeEvents.

Phase 14.1 verification: `npm run typecheck` passed; `npm run lint` passed with zero warnings; `npm test` passed 32 files / 238 tests; `npm run test:e2e` passed 43/43 Chromium tests; `npm run build` passed with the existing route set. The new contract contributes 3 test files / 14 tests and no browser route.
...
The Life Map Evolution follow-up turns 30/90/365 into sediment depth rather than a select filter. Range changes interpolate the previous and next Canvas terrain for 760ms; Lens changes remain immediate and reduced-motion users receive the final frame. Absolute event frequency controls grain density, accumulated duration controls territory size/contour weight, and the Places lens renders temporal affinity as trajectory traces. Adjacent details now describe theme, time span, recent change and related directions before showing count/duration footprints. This remains presentation-only: LifeEvent, Dexie, repositories and Life Statistics are unchanged.

Evolution verification: `npm run typecheck` passed; `npm run lint` passed with zero warnings; `npm test` passed 29 files / 224 tests; `npm run test:e2e` passed 43/43 Chromium tests; `npm run build` passed and prerendered `/life`. The 30-day, one-year, Places, real-data and 390px captures are stored under `design/life-visualization/`.

The typography follow-up removes region-name ellipsis and separates text from dense terrain with a restrained paper-tone halo. Active labels rise above neighboring regions. On narrow screens the adjacent detail is clamped inside the viewport and placed above or below its region. Geometric E2E checks now reject label scroll clipping, viewport escape and pairwise label overlap across all three lenses at 320px. The complete five-command gate remains green after this correction.
