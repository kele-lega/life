...
for (const colorScheme of ["light", "dark"] as const) {
  test(`homepage UI-1 ${colorScheme}: local recording, full text and responsive controls`, async ({ page }, testInfo) => {
    ...
    await expect(navigation.getByRole("link", { name: "时间线", exact: true })).toHaveCSS("font-weight", "500");
    await expect(navigation.getByRole("link", { name: "生活地图", exact: true })).toHaveAttribute("href", "/life");
    await expect(page.locator("main")).toHaveCSS("background-color", colorScheme === "light" ? "rgb(250, 250, 249)" : "rgb(23, 26, 25)");
...
test("product navigation keeps every existing destination reachable at all sizes", async ({ page }, testInfo) => {
  ...
  for (const width of [320, 390, 430, 768, 1440]) {
    ...
    const navigation = page.getByRole("navigation", { name: mobile ? "底部导航" : "主导航", exact: true });
    for (const [label, href] of [["记录", "/"], [mobile ? "回看" : "时间线", "/timeline"], ["日记", "/diary"], ["生活地图", "/life"]]) {
      await expect(navigation.getByRole("link", { name: label, exact: true })).toHaveAttribute("href", href);
    }
    const recall = page.getByRole("navigation", { name: "浏览生活", exact: true });
    await expect(recall.getByRole("link", { name: "日历", exact: true })).toHaveAttribute("href", "/calendar");
    await expect(recall.getByRole("link", { name: "搜索", exact: true })).toHaveAttribute("href", "/search");

    await noOverflow(page);
    await targetsFit(page);
    if (width === 390 || width === 1440) {
      await page.screenshot({ path: testInfo.outputPath(`product-navigation-${width}.png`), fullPage: true });
    }

    await recall.getByRole("link", { name: "日历", exact: true }).focus();
    await expect(recall.getByRole("link", { name: "日历", exact: true })).toBeFocused();
  }
});
