...
for (const colorScheme of ["light", "dark"] as const) {
  test(`shared ${colorScheme} theme follows navigation and system changes`, async ({ page }, testInfo) => {
    const surface = colorScheme === "dark" ? "rgb(23, 26, 25)" : "rgb(250, 250, 249)";
    const ink = colorScheme === "dark" ? "rgb(237, 240, 235)" : "rgb(35, 38, 37)";
    ...
    for (const route of ["/", "/timeline", "/calendar", "/diary", "/search", "/diary/new"]) {
      ...
      if (route === "/calendar") {
        ...
        await expect(day).toHaveCSS("background-color", colorScheme === "dark" ? "rgb(184, 80, 70)" : "rgb(158, 75, 62)");
    ...
    await expect(page.locator("body")).not.toHaveCSS("background-color", surface);
    await expect(body).not.toHaveCSS("color", ink);
